...
def _find_combinations(values, target, species_list, tolerance, mw_dict, bonds_allowed, si_units, n_units, verbose):
            ...
            if has_si and has_n:
                ...
                if n_stubs == si_stubs and n_stubs > 0:
                    ...
                    if verbose and count % 500000 == 0 and n_stubs != si_stubs:  # Only show occasionally
                        print(
                            f"Rejected {combo_dict}: Si stubs ({si_stubs}) ≠ N stubs ({n_stubs}) - cannot form Si-N only bonds")
            else:
    ...
    if verbose:
        print(
            f"Combination search complete: {len(results)} valid combinations found out of {total_combinations:,} checked")
...
def _validate_molecule_strict_si_n(mol, species_list_flat, si_units, n_units):
    ...
    for bond in mol.GetBonds():
        ...
        if a_symbol == 'Si' and b_symbol == 'N':
            ...
            invalid_bonds.append(
                f"FORBIDDEN bond (non Si-N): {a_idx}({a_symbol},{a_species}) - {b_idx}({b_symbol},{b_species})")
    ...
    if not is_valid and len(invalid_bonds) == 0:
        ...
        if total_heavy_bonds == 0:
            ...
            invalid_bonds.append(
                f"Not all heavy bonds are Si-N: {si_n_bonds} Si-N bonds out of {total_heavy_bonds} total heavy bonds")
...
def _generate_molecules_for_combo(combo, num_mols, output_dir, max_estimate_threshold,
    ...
    if estimated_molecules == -1:
        if verbose:
            print(
                f"Combination {combo}: estimated >>{uncertainty_threshold} unique molecular structures (many possible), generating {num_mols}")
        ...
        if estimated_molecules <= 10:
            ...
            if verbose:
                print(
                    f"Small structure space for {combo}: estimated {estimated_molecules} unique molecules, generating {num_mols} (targeting all unique structures)")
        elif estimated_molecules <= 50:
            num_mols = min(num_mols,
                           int(estimated_molecules * 1.2))  # Generate 120% to account for estimation uncertainty
            if verbose:
                print(
                    f"Moderate structure space for {combo}: estimated {estimated_molecules} unique molecules, generating {num_mols} (targeting most unique structures)")
        elif estimated_molecules <= 200:
            num_mols = min(num_mols, int(estimated_molecules * 0.8))  # Generate 80% for larger spaces
            if verbose:
                print(
                    f"Large structure space for {combo}: estimated {estimated_molecules} unique molecules, generating {num_mols} (targeting 80%)")
        else:
            num_mols = min(num_mols, int(estimated_molecules * 0.5))  # Generate 50% for very large spaces
            if verbose:
                print(
                    f"Very large structure space for {combo}: estimated {estimated_molecules} unique molecules, generating {num_mols} (targeting 50%)")
        ...
        if verbose:
            print(
                f"Extremely large structure space for {combo}: estimated >>{uncertainty_threshold if estimated_molecules > uncertainty_threshold else estimated_molecules} unique molecules possible, limiting to {num_mols}")
    else:
        if estimated_molecules > uncertainty_threshold:
            ...
            if verbose:
                print(
                    f"Combination {combo}: estimated {estimated_molecules} unique molecular structures, generating {num_mols}")
    ...
    if n_stubs != si_stubs:
        if verbose:
            print(
                f"Skipping {combo_name}: unmatched N stubs ({n_stubs}) vs Si stubs ({si_stubs}). Si-N only bonding requires equal stub counts.")
        return 0
    ...
    if not (has_si and has_n):
        if verbose:
            print(
                f"Skipping {combo_name}: missing either Si atoms ({has_si}) or N atoms ({has_n}). Si-N bonding requires both.")
        return 0
    ...
    if verbose:
        print(
            f"Generated {generated}/{num_mols} molecules for combo {combo_name} (attempts: {attempts}, unique structures: {actual_unique}, estimated unique possible: {estimated_display})")
        print(f"  CONSTRAINT CHECK: All molecules have ONLY Si-N bonds (no Si-Si or N-N bonds)")
        print(
            f"  UNIQUENESS: Generated {actual_unique} unique SMILES out of estimated {estimated_display} possible unique structures")
...
if __name__ == '__main__':
    # Example: Generate molecules with target MW 326.928 using enhanced algorithm
    stats = generate_sin_only_molecules(
        target_mw=473.352,
        output_dir='test',
        tolerance=0,
